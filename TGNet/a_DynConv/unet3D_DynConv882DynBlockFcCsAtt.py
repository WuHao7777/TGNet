...
class ChannelSpatialAttention(nn.Module):
    def __init__(self,Fg,Fl):
        ...
        self.conv1=nn.Sequential(
            ...
            Conv3d(Fl,Fl,kernel_size=1,stride=1,padding=0,bias=False)
        )
        ...
        self.Fl=Fl
    def parse_att_params(self,params,channels,weight_nums):
        assert params.dim()==2

        assert params.size(1)==sum(weight_nums)
        
        num_insts=params.size(0)
        params_splits=list(torch.split_with_sizes(params,weight_nums,dim=1))
        weight_splits=params_splits[0]
        weight_splits=weight_splits.reshape(num_insts*channels,-1,1,1,1)

        return weight_splits
    
    
    def DynAtt_forward(self,features,weights,groups):
        assert features.dim()==5
        x=features
        x=F.conv3d(x,weights,stride=1,padding=0,
                   groups=groups)
        return x 
    ...
    def forward(self,xl,g,task_encoding):
        ...
        paramsAtt=self.controller(g_cond)
        paramsAtt.squeeze_(-1).squeeze_(-1).squeeze_(-1)
        weight=self.parse_att_params(paramsAtt,_,[self.Fl])
        logits=self.DynAtt_forward(features,weight,N*_)
        logits=logits.reshape(N,_,D,H,W)
...
class unet3D(nn.Module):
    def __init__(self, layers, num_classes=3, weight_std = False):
        ...
        self.SAtt3=ChannelSpatialAttention(256,256)
        self.SAtt2=ChannelSpatialAttention(128,128)
        self.SAtt1=ChannelSpatialAttention(64,64)
        self.SAtt0=ChannelSpatialAttention(32,32)
    ...
    def forward(self, input, task_id):
        ...
        skip3=self.SAtt3(skip3,xl3,task_encoding)
        ...
        skip2=self.SAtt2(skip2,xl2,task_encoding)
        ...
        skip1=self.SAtt1(skip1,xl1,task_encoding)
        ...
        skip0=self.SAtt0(skip0,xl0,task_encoding)
