...
def main():
    ...
    with Engine(custom_parser=parser) as engine:
        args = parser.parse_args()  #type(args)=argparser.namespace
        if args.num_gpus > 1:
        ...
        loss_seg_DICE = loss.DiceLoss4MOTS(num_classes=args.num_classes).to(device) #mean loss
        ...
        for epoch in range(args.num_epochs):
            ...
            epoch_loss = []
            adjust_learning_rate(optimizer, epoch, args.learning_rate, args.num_epochs, args.power) # base_lr * ((1 - float(iter) / max_iter) ** (power))
            ...
            for iter, batch in enumerate(trainloader):
                ...
                # its a batch, each batch containing 2 samples, eg. images.shape=[2,1,64,192,192], labels.shape=[2,2,64,192,192] name=['case0000', 'liver_1'] task_ids=[1,0]] batch is a dict containing {image,label,name,task_id}
                optimizer.zero_grad()
                ...
                reduce_Dice = engine.all_reduce_tensor(term_seg_Dice) #return torch.mean
                ...
                epoch_loss.append(float(reduce_all)) #seems reduce_all is loss
            ...
            epoch_loss = np.mean(epoch_loss) #epoch mean loss
